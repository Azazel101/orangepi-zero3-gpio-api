...
async def get_loxone_output_template():
    ...
    xml_lines = [
        '<?xml version="1.0" encoding="utf-8"?>',
        '<Templates>',
        f'  <VirtualOutTemplate Title="OrangePi_GPIO_Outputs ({ip_addr})" Comment="Generated by API" Icon="056">',
        f'    <VirtualOutConnector Name="Orange Pi Zero 3" Address="{base_url}" CmdEvent="">'
    ]
    ...
    for pin_cfg in config["pins"]:
        ...
        if pin_cfg.get("direction", "output").lower() == "output":
            ...
            xml_lines.append(
                f'      <VirtualOutCmd Title="Pin {pin_num} ({name}) ON" Action="/pins/set" Method="POST" HTTPPostData="{on_body}" />'
            )
            xml_lines.append(
                f'      <VirtualOutCmd Title="Pin {pin_num} ({name}) OFF" Action="/pins/set" Method="POST" HTTPPostData="{off_body}" />'
            )

    xml_lines.append('    </VirtualOutConnector>')
    xml_lines.append('  </VirtualOutTemplate>')
    xml_lines.append('</Templates>')
